	...
	while true do
		if selling then
			if secondsRemaining > 0 then
				...
				sendNotification('Pozostały czas oczekiwania: '..secondsRemaining..' sekund/y.', 'info', 1000)
			end
			Citizen.Wait(1000)
		end
		Citizen.Wait(100)
									...
									if distanceFromCity < Config.DistanceFromCity then
										if playerHasDrugs and sold == false and selling == false then 
											...
											if random == Config.PedRejectPercent then
												sendNotification('Osoba odrzuciła twoją ofertę.', 'error', 5000)
												oldped = ped
												--PED CALLING COPS
										...
										sendNotification('Jesteś zbyt daleko od miasta.', 'error', 5000)
									end
								end
							end
						end
...
Citizen.CreateThread(function()
	while true do
		...
		if selling then
			...
			--TOO FAR
			if distance > 5 then
				sendNotification('Oddaliłeś się od klienta, sprzedawanie anulowane.', 'warning', 5000)
				selling = false
			...
			if secondsRemaining <= 1 then
				...
				StopAnimTask(oldped, "mp_common","givetake1_a", 1.0)
				playerHasDrugs = false
				sold = false
				TriggerServerEvent('sellDrugs')
			end	
			
			if secondsRemaining == 1 and Config.PlayAnimation then
				RequestAnimDict("mp_common")
				while (not HasAnimDictLoaded("mp_common")) do 
					Citizen.Wait(0) 
				end
				TaskPlayAnim(pid,"mp_common","givetake1_a",100.0, 200.0, 0.3, 120, 0.2, 0, 0, 0)
				TaskPlayAnim(oldped,"mp_common","givetake1_a",100.0, 200.0, 0.3, 120, 0.2, 0, 0, 0)
			end
		end	
	end
end)	
...
AddEventHandler('nomoredrugs', function()
	sendNotification('Nie posiadasz przy sobie żadnego towaru.', 'error', 5000)
	playerHasDrugs = false
	sold = false
	selling = false
	secondsRemaining = 0
...
AddEventHandler('playerhasdrugs', function()
	-- sendNotification('Wciśnij "E", aby zaproponowac osobie narkotyki.', 'info', 5000)
	playerHasDrugs = true
end)
