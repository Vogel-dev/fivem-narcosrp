...
AddEventHandler('esx_repairkit:onUse', function()
	...
	if IsAnyVehicleNearPoint(coords.x, coords.y, coords.z, 5.0) then
		...
		if DoesEntityExist(vehicle) then
			...
			Citizen.CreateThread(function()
				...
				if CurrentAction ~= nil then
					...
					sendNotification('Pojazd został poprawnie naprawiony.', 'success', 5000)
				end
		...
		Citizen.CreateThread(function()
			...
			if CurrentAction ~= nil then
				...
				if IsControlJustReleased(0, Keys["X"]) then
					...
					sendNotification('Naprawa została przerwana.', 'warning', 5000)
					CurrentAction = nil
				end
			end
		...
		sendNotification('W pobliżu nie znajduje się żaden pojazd.', 'warning', 5000)
	end
end)
