	...
	ESX.UI.Menu.Open(
		...
		function(data, menu)
			...
			if string.len(plate) < 8 or string.len(plate) > 8 then
				sendNotification('Taki numer rejestracyjny nie istnieje!', 'error', 5000)
			else
				ESX.TriggerServerCallback('flux_garages:checkIfPlayerIsOwner', function(isOwner)
					if isOwner then
							...
							function(data2, menu2)
								if data2.current.value == 'give_sub' then
									...
									if closestPlayer ~= -1 and closestDistance <= 3.0 then
										...
										sendNotification('Brak osób w!', 'error', 5000)
									end
								elseif data2.current.value == 'manage_sub' then
									ESX.TriggerServerCallback('flux_garages:getSubowners', function(subowners)
										if #subowners > 0 then
											...
											sendNotification('Brak współwłaścicieli dla tego pojazdu!', 'error', 5000)
										end
									end, plate)
								end
							end,
						...
						sendNotification('Nie jesteś właścicielem tego pojazdu!', 'error', 5000)
					end
				end, plate)
			end
		end,
			...
			if IsControlPressed(0, 38) and (GetGameTimer() - GUI.Time) > 300 then
				if CurrentAction == 'park_car' then
					...
					if health > Config.MinimumHealth then
						...
						sendNotification('Pojazd musi być naprawiony!', 'warning', 5000)
					end
				elseif CurrentAction == 'pullout_car' then
					SendNUIMessage({
						clearme = true
...
RegisterNUICallback('towCar', function(data, cb)
	...
	ESX.TriggerServerCallback('flux_garages:towVehicle', function(id)
		if id ~= nil then
			...
			sendNotification('Trwa poszukiwanie pojazdu...', 'info', 3000)
			...
			if entity == 0 then
				ESX.TriggerServerCallback('flux_garages:checkMoney', function(hasMoney)
					if hasMoney then
						...
						sendNotification('Pomyślnie odholowałeś pojazd o numerze rejestracyjnym '..data.model..'.', 'success', 6000)
					else
						sendNotification('Trwa poszukiwanie pojazdu...', 'info', 3000)
					end
				end)
			elseif entity ~= 0 and (GetVehicleNumberOfPassengers(entity) > 0 or not IsVehicleSeatFree(entity, -1)) then
				sendNotification('TNie możesz odholować tego pojazdu', 'error', 5000)
			else
				ESX.TriggerServerCallback('flux_garages:checkMoney', function(hasMoney)
					if hasMoney then
						TriggerServerEvent('flux_garages:pay')
						...
						sendNotification('Nie stać Cię na odholowanie tego pojazdu.', 'warning', 3000)
					end
				end)
			end
		else
