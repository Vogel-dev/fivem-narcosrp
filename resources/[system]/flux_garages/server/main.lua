...
AddEventHandler('flux_garages:removeCarFromParking', function(plate, networkid)
	...
	if plate ~= nil then
		...
		sendNotification(xPlayer.source, 'Pojazd został pomyślnie wyciągnięty.', 'success', 5000)
	end
end)
					...
					MySQL.Async.fetchAll(
						...
						function(count)
							if #count >= Config.MaxSubs then
								sendNotification(xPlayer.source, 'Osiągnięto maksymalną ilość współwłaścicieli dla tego pojazdu.', 'warning', 5000)
							else
								MySQL.Sync.execute(
								...
								sendNotification(xPlayer.source, 'Dodano nowego współwłaściciela.', 'success', 5000)
								sendNotification(xPlayer.source, 'Zostałeś współwłaścicielem pojazdu o numerach rejestracyjnych '..plate..'.', 'info', 5000)
							end
						end
					)
				else
					sendNotification(xPlayer.source, 'Nie jesteś właścicielem tego pojazdu.', 'error', 5000)
				end
			else
				sendNotification(xPlayer.source, 'Taki numer rejestracyjny nie istnieje!', 'error', 5000)
			end
		end
	)
end)
...
AddEventHandler('flux_garages:deleteSubowner', function(plate, identifier)
	...
	sendNotification(xPlayer.source, 'Usunięto współwłaściciela.', 'success', 5000)
end)
