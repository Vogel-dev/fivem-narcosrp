...
Citizen.CreateThread(function ()
    while true do
        ...
        if inRangeToActivePhone == true and currentPlaySound == false and Phone == true  then
  ...
  while true do
    Citizen.Wait(0)
      if ( IsControlJustReleased( 0, 288 ) or IsDisabledControlJustReleased( 0, 288 ) ) and GetLastInputMethod( 2 ) then
        TriggerServerEvent('gcphone:TryOpenPhone')
        end
...
AddEventHandler("gcPhone:receiveMessage", function(message)
    ...
    if message.owner == 0 then
        local text = '~y~Otrzymałeś nową wiadomość...'
        if ShowNumberNotification == true then
            text = '~y~Otrzymałeś nową wiadomość od ~r~'.. message.transmitter
            for _,contact in pairs(contacts) do
                if contact.number == message.transmitter then
                    text = '~y~Otrzymałeś nową wiadomość od ~r~'.. contact.display
                    break
                end
            end
        end
