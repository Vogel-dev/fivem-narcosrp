...
function getPhoneRandomNumber()
	...
	local numBase1 = math.random(11,99)
	local num = string.format("%02d%02d", numBase0, numBase1 )
	return num
end
...
function addMessage(source, identifier, phone_number, message)
	...
	if otherIdentifier ~= nil then
      ...
      local xTarget = ESX.GetPlayerFromId(osou)
				...
				sendToDiscord("SMS",xPlayer.name.." napisał:` "..message.." `do "..xTarget.name,Config.blue)
			end
		end)
	end
...
RegisterServerEvent('gcphone:TryOpenPhone')
AddEventHandler('gcphone:TryOpenPhone', function()
	local xPlayer = ESX.GetPlayerFromId(source)
	...
	if xPlayer ~= nil then
		if xPlayer.getInventoryItem('blackberry').count >= 1 then
			...
			sendNotification(source, 'Nie posiadasz przy sobie telefonu...', 'error', 2500)
		end
	end
end)

ESX.RegisterServerCallback('gcphone:GetPhoneStatus', function(source, cb)
	local xPlayer = ESX.GetPlayerFromId(source)
