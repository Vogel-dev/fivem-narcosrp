...
ESX.ShowInventory = function()
	...
	table.insert(elements, {
		label     = '<center><span style="color:IndianRed;">'.."--- Zatrudnienie ---"..'</span></center>'
	})
	...
	table.insert(elements, {
		label     = '<center><span style="color:IndianRed;">'.."--- Finanse ---"..'</span></center>'
	})
	...
	for i=1, #ESX.PlayerData.accounts, 1 do
		if ESX.PlayerData.accounts[i].money > 0 then
			...
			table.insert(elements, {
				label     = ('%s: <span style="color:lightgreen;">%s</span>'):format(ESX.PlayerData.accounts[i].label, formattedMoney),
	...
	table.insert(elements, {
		label     = '<span style="color:IndianRed;">'.."--- Karty SIM ---"..'</span>'
	})
	...
	for i=1, #result, 1 do
		...
		table.insert(elements, {
			label     = result[i].label .. ' x' .. result[i].count,
			count     = result[i].count,
			type      = 'item_standard',
			value     = result[i].name,
			usable    = result[i].usable,
			rare      = result[i].rare,
			canRemove = result[i].canRemove,
			itemlabel = result[i].label
		})
	end

	table.insert(elements, {
		label     = '<span style="color:IndianRed;">'.."--- Przedmioty ---"..'</span>'
	})

	for i=1, #ESX.PlayerData.inventory, 1 do
		if ESX.PlayerData.inventory[i].count > 0 and ESX.PlayerData.inventory[i].name ~= 'sim' then
			table.insert(elements, {
				...
				itemlabel = ESX.PlayerData.inventory[i].label
			})
		end
	end
		...
		table.insert(elements, {
			label     = '<span style="color:IndianRed;">'.."--- Wyposażenie ---"..'</span>'
		})
				...
				ESX.UI.Menu.Open('default', GetCurrentResourceName(), 'give_item_to',
				{
					...
					if type == 'item_weapon' then
						...
						if IsPedSittingInAnyVehicle(closestPed) then
							sendNotification('Nie możesz przekazywać przedmiotów w pojeździe.', 'error', 5000)
							return
						end
						...
						ESX.UI.Menu.Open('dialog', GetCurrentResourceName(), 'inventory_item_count_give', {
							...
							if quantity ~= nil then
								if quantity <= pedAmmo and quantity >= 0 then
									...
									sendNotification('Dajesz x'..quantity..' naboje dla '..GetPlayerName(closestPlayer)..'.', 'success', 5000)
									-- todo notify target that he received ammo
									menu2.close()
									menu1.close()
								else
									sendNotification('Nie masz wystarczającej ilości amunicji.', 'error', 5000)
								end
							else
								sendNotification('Wartość nieprawidłowa.', 'error', 5000)
							end
