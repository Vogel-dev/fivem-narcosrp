  ...
  if IsPedArmed(ped, 4) then
    ...
    if hash~=nil then
      ...
      AddAmmoToPed(GetPlayerPed(-1), hash,25)
        sendNotification('Użyłeś magazynku do broni...', 'success', 5000)
    else
      sendNotification('Nie masz w ręku broni...', 'warning', 5000)
    end
  else
      sendNotification('Ten rodzaj amunicji nie jest odopwiedni do tej broni.', 'error', 5000)
  end
end)
