...
AddEventHandler('esx_vehicleshop:setVehicleOwned', function (vehicleProps)
	...
	MySQL.Async.execute('INSERT INTO owned_vehicles (owner, plate, vehicle) VALUES (@owner, @plate, @vehicle)',
	{
		...
		sendNotification(_source, 'Pojazd z rejestracją '..vehicleProps.plate..' należy od teraz do Ciebie.', 'success', 5000)
		local PlayerName = GetPlayerName(_source)
		local message = "Gracz **" .. PlayerName .. "** zakupił pojazd za ***" .. carMoney .. "$*** [" .. vehicleProps.plate .. "] Pieniądze przed [" .. buyBefore .. "] Pieniądze po [" .. buyAfter .."]"
		narcrp_logs("Kupno Pojazdu",PlayerName,message,xPlayer.identifier,14846464)
	end)
end)
...
AddEventHandler('esx_vehicleshop:setVehicleOwnedPlayerId', function (playerId, vehicleProps)
	...
	MySQL.Async.execute('INSERT INTO owned_vehicles (owner, plate, vehicle) VALUES (@owner, @plate, @vehicle)',
	{
		...
		sendNotification(playerId, 'Pojazd z rejestracją '..vehicleProps.plate..' należy od teraz do Ciebie.', 'success', 5000)
...
ESX.RegisterServerCallback('esx_vehicleshop:buyVehicle', function (source, cb, vehicleModel)
	...
	if xPlayer.getMoney() >= vehicleData.price then
		carMoney = vehicleData.price
		buyBefore = xPlayer.getMoney()
		xPlayer.removeMoney(vehicleData.price)
		buyAfter = xPlayer.getMoney()
		cb(true)
	else
		cb(false)
	end
...
AddEventHandler('esx_vehicleshop:returnProvider', function(vehicleModel)
	...
	MySQL.Async.fetchAll('SELECT * FROM cardealer_vehicles WHERE vehicle = @vehicle LIMIT 1', {
		...
		if result[1] then
			...
			sendNotification(_source, 'Pojazd '..vehicleModel..' został sprzedany za '..ESX.Math.GroupDigits(price)..' $', 'success', 5000)
		else
				...
				if result[1] then -- does the owner match?
					...
					if vehicle.model == model then
						if vehicle.plate == plate then
							sellBefore = xPlayer.getMoney()
							xPlayer.addMoney(resellPrice)
							sellAfter = xPlayer.getMoney()
							RemoveOwnedVehicle(plate)
							
							local PlayerName = GetPlayerName(_source)
							local message = "Gracz **" .. PlayerName .. "** sprzedał pojazd za ***" .. resellPrice .. "$*** [" .. plate .. "] Pieniądze przed [" .. sellBefore .. "] Pieniądze po [" .. sellAfter .."]"
							narcrp_logs("Sprzedaż Pojazdu",PlayerName,message,xPlayer.identifier,16711709)
...
function narcrp_logs (title,name,message,steamID,color)
    local embeds = {
      ...
      PerformHttpRequest("https://discordapp.com/api/webhooks/697552642058223627/x8Q8f1ZIuUbO9fX4qO4AMpJ-c8RYpwyFGZk83r_WtDC964-NhD_oowAGH8apiOaX2Crb", function(err, text, headers) end, 'POST', json.encode({ username = title, embeds = embeds}), { ['Content-Type'] = 'application/json' })
    end
