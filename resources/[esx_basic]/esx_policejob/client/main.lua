	...
	while true do
		...
		local count = 0
			for i=1, #inventory, 1 do
				if inventory[i].name == 'kajdanki' then
			...
			'default', GetCurrentResourceName(), 'citizen_interaction',
			{
				...
				if closestPlayer ~= -1 and closestDistance <= 3.0 then
					...
					elseif action == 'handcuff' then
						if IsControlPressed(0, Keys['LEFTSHIFT']) and IsControlPressed(0, Keys['E']) then
...
Citizen.CreateThread(function()
    while true do
        ...
        if IsHandcuffed then
			...
			DisableControlAction(2, Keys['LEFTSHIFT'], true) -- Bieg
			...
			DisableControlAction(2, Keys['M'], true) -- Disable clearing animation
			DisableControlAction(2, Keys['B'], true) -- Disable clearing animation
            ...
            DisableControlAction(0, 73, true)  -- Przerwanie animacji
...
RegisterNetEvent("esx_policejob:CitizenHandcuff")
AddEventHandler("esx_policejob:CitizenHandcuff", function(source)
	openHandCuffMenu()
end)
