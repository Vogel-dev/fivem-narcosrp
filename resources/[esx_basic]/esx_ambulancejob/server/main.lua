...
if Config.EarlyRespawn and Config.EarlyRespawnFine then
	...
	ESX.RegisterServerCallback('esx_ambulancejob:payFine', function(source, cb)
		local xPlayer = ESX.GetPlayerFromId(source)
		sendNotification(xPlayer.source, 'Zapłaciłeś '..Config.EarlyRespawnFineAmount..' $, aby się odrodzić.', 'info', 5000)
...
AddEventHandler('esx_ambulancejob:removeItem', function(item)
	...
	if item == 'bandage' then
		sendNotification(_source, 'Użyłeś bandaż...', 'success', 5000)
	elseif item == 'medikit' then
		sendNotification(_source, 'Użyłeś apteczki...', 'success', 5000)
	end
end)
...
AddEventHandler('esx_ambulancejob:giveItem', function(item)
	...
	if qtty < limit then
		...
		sendNotification(_source, 'Posiadasz już maksymalną ilość.', 'warning', 5000)
	end
end)
...
TriggerEvent('es:addGroupCommand', 'revive', 'mod', function(source, args, user)
	...
	TriggerClientEvent('chat:addMessage', source, { args = { '^1SYSTEM', 'Brak uprawnień.' } })
