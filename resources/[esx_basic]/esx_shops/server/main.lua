...
AddEventHandler('esx_shops:buyItem', function(itemName, amount, zone)
	...
	if xPlayer.getMoney() >= price then
		...
		if sourceItem.limit ~= -1 and (sourceItem.count + amount) > sourceItem.limit then
			sendNotification(_source, 'Nie posiadasz wystarczająco miejsca w ekwipunku.', 'warning', 5000)
		else
			xPlayer.removeMoney(price)
			xPlayer.addInventoryItem(itemName, amount)
			sendNotification(_source, 'Zakupiłeś '..itemLabel..' w ilości '..amount..' za cenę '..ESX.Math.GroupDigits(price)..' $', 'success', 5000)
		end
	else
		local missingMoney = price - xPlayer.getMoney()
		sendNotification(_source, 'Nie posiadasz wystarczająco pieniędzy, brakuje Ci: '..ESX.Math.GroupDigits(missingMoney)..' $', 'warning', 5000)
	end
end)
